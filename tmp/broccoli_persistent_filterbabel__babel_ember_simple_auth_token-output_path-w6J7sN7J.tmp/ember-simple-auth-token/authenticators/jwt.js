  ...
  exports['default'] = _emberSimpleAuthTokenAuthenticatorsToken['default'].extend({
    ...
    */
    authenticate: function authenticate(credentials, headers) {
      ...
      return new _ember['default'].RSVP.Promise(function (resolve, reject) {
        ...
        _this2.makeRequest(_this2.serverTokenEndpoint, data, headers).then(function (response, status, qhr) {
          _ember['default'].run(function () {
            try {
              var sessionDataUnparsed = _this2.handleAuthResponse(qhr);
              var sessionData = {};
              var tok = sessionDataUnparsed.getResponseHeader(_this2.authorizationHeaderName).replace("Bearer", "");
              var tokenData = _this2.getTokenData(tok);
              sessionData[_this2.tokenPropertyName] = tok;
              sessionData["username"] = _ember['default'].get(tokenData, "sub");
              sessionData["role"] = _ember['default'].get(tokenData, "role");
              sessionData["userid"] = _ember['default'].get(tokenData, "userid");
    ...
    */
    ...
    handleAuthResponse: function handleAuthResponse(response) {
      ...
      tokenExpireData["email"] = _ember['default'].get(tokenData, "email");
      tokenExpireData["name"] = _ember['default'].get(tokenData, "name");
      tokenExpireData["role"] = _ember['default'].get(tokenData, "role");
